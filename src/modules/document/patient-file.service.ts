...
export class PatientFileService {
  ...
  /**
   * *** ADDITIVE (M-15). THE GENERATED-PRESCRIPTION WRITE PATH (FR-9.5,
   * FR-14.2). ***
   *
   * The counterpart to `validateCategory`'s by-name rejection of
   * `prescription_pdf` on the upload path: a prescription is GENERATED by the
   * platform from a finalised clinical record and reaches this table through
   * here, never through HTTP. Keeping both halves is what makes "the facade is
   * the only door" true rather than merely intended.
   *
   * IDEMPOTENT ON `consultationId`, and the check runs BEFORE any storage call:
   * an existing non-deleted `prescription_pdf` is returned unchanged and the
   * caller\'s buffer is discarded, so a retried finalise costs one indexed SELECT
   * and stores nothing. Without that, every retry would mint a second object in
   * the bucket and a second row, and the patient would see two prescriptions
   * for one consultation with no way to tell which is current.
   *
   * `uploadedByDoctorId` is set to the treating doctor, which has a
   * consequence worth stating: `deleteOwn` refuses any file with that column
   * set, so a patient can read and download their prescription but cannot
   * delete it. That is the correct asymmetry for a clinical record and it falls
   * out of a rule that already existed.
   *
   * NO CONTENT SNIFFING. `resolveVerifiedContentType` exists because a client\'s
   * declared type is a header a client wrote; these bytes were produced by
   * `pdfkit` inside this process one call ago, so there is nothing to verify
   * and nobody to distrust. NO SIZE CHECK either, for the same reason — the
   * business cap is a rule about what a PATIENT may upload, and refusing to
   * store a doctor\'s own finalised prescription because an admin lowered
   * `documents.max_file_size_mb` would be the tail wagging the dog.
   */
  async writePrescriptionPdf(input: {
    consultationId: string;
    patientId: string;
    doctorId: string | null;
    fileName: string;
    pdf: Buffer;
  }): Promise<SafePatientFileRow> {
    const existing = await this.repo.findByConsultationAndCategory(input.consultationId, 'prescription_pdf');
    if (existing) {
      return toSafePatientFileRow(existing);
    }
    ...
    const created = await this.repo.create({
      ...
      uploadedByDoctorId: input.doctorId,
    ...
    await this.audit.write({
      actorType: input.doctorId ? 'doctor' : 'system',
      actorId: input.doctorId,
      action: 'create',
      entityType: DOCUMENT_AUDIT_ENTITY_TYPES.PATIENT_FILE,
      entityId: created.id,
      consultationId: input.consultationId,
      metadata: { fileCategory: 'prescription_pdf', generated: true },
    });
