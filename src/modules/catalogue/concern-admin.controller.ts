 ...
 * `doctor-admin.controller.ts`. Gated by the concern-specific
 * `CONCERNS_READ`/`CONCERNS_MANAGE` (not the borrowed `SPECIALTIES_READ`/
 * `SPECIALTIES_MANAGE`), so the admin-UI-facing permission descriptions
 * accurately name what's actually being granted.
 */
...
export class ConcernAdminController {
  ...
  @Get()
  @RequirePermission(PERMISSIONS.CONCERNS_READ)
  ...
  @Post()
  @RequirePermission(PERMISSIONS.CONCERNS_MANAGE)
  ...
  @Patch(':id')
  @RequirePermission(PERMISSIONS.CONCERNS_MANAGE)
  update(@CurrentUser() auth: AuthContext, @Param('id', createUuidValidationPipe('id')) id: string, @Body() dto: UpdateConcernDto) {
  ...
  updateMapping(
    ...
    @Body() dto: UpdateConcernMappingDto,
  ) {
