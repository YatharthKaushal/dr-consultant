...
/**
 * `app_config` access for the `payments.*` keys, and ONLY those.
 *
 * A direct counterpart of `search-config.repository.ts`, for the same reason
 * it gives: `app_config` is not owned by any one module, so the boundary rule
 * that applies is `docs/MODULES.md` §7 — "Configuration lives with its owning
 * module and is edited from the admin panel: ... fee and GST in M-12." M-12
 * therefore owns the `payments.*` keys and nothing else, and
 * `payment-config.service.ts` refuses to write anything outside that set
