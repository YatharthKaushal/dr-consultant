...
- availability: 43 cases, all pass against current code. No bugs found. Covers the weekly-schedule atomic replace, override/block shape and overlap rules, the `RULE_NOT_FOUND` ownership-leak collapse (another doctor's rule id gets the identical 404 a nonexistent id gets, confirmed by code and by test), the real `scheduling.min_notice_minutes`/`scheduling.max_slot_query_days` trap read from `app_config` rather than hardcoded, an unbookable doctor's slot routes degrading to `[]` rather than an error, and the `AVAILABILITY_READ` vs `AVAILABILITY_MANAGE` split on the admin settings-override route.
